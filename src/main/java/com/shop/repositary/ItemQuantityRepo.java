...
public interface ItemQuantityRepo extends JpaRepository<ItemQuantity, ItemQuantityKey> {
	...
	@Query(value = "SELECT iq.order_id,iq.quantity,itm.name,itm.brand,dr.name FROM shop.item_quantity iq INNER JOIN shop.item itm ON iq.shop_id = itm.shop_id  INNER JOIN shop.orders odr ON iq.order_id = odr.order_id INNER JOIN shop.delivery_rider dr ON odr.delivery_id = dr.user_id WHERE iq.shop_id = :id", nativeQuery = true)
